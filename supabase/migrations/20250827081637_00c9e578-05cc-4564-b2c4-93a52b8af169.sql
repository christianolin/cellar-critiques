...
CREATE POLICY IF NOT EXISTS "Authenticated users can update wines"
ON public.wines
FOR UPDATE
TO authenticated
USING (true)
...
CREATE POLICY IF NOT EXISTS "Composition viewable by everyone"
ON public.wine_grape_composition
FOR SELECT
USING (true);

CREATE POLICY IF NOT EXISTS "Authenticated can insert composition"
ON public.wine_grape_composition
FOR INSERT
TO authenticated
WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Authenticated can update composition"
ON public.wine_grape_composition
FOR UPDATE
TO authenticated
USING (true)
...
CREATE POLICY IF NOT EXISTS "Users manage their own consumptions"
